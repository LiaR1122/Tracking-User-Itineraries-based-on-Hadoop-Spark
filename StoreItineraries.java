...
public class StoreItineraries {
    ...
    // input: log files
    // output: <phone_number_YY-MM-DDDD, LAC:CELL>
    public static class LogsMapper
            extends Mapper<Object, Text, Text, Text> {
        private Text cell = new Text();
        private Text lac = new Text();
        private Text phone_number_and_date = new Text();
        ...
        public void map(Object key, Text value, Context context
            ...
            if (strList.length > 4) {
                lac.set(strList[1]);
                cell.set(strList[2]);
                phone_number_and_date.set(strList[3] + "_" + strList[4].substring(0, 10));
                identifier.add(lac.toString());
                identifier.add(cell.toString());
                context.write(phone_number_and_date, new Text(String.join(":", identifier)));
            }
        }
    }

    // Job2 Reduce: write result to HBase
    // input: <phone_number_YY-MM-DDDD, LAC:CELL>
    // output: Table "UserTimeItinerary"
    ...
    public static void main(String[] args) throws IOException {
        ...
        createSchemaTables(config, "UserTimeItinerary", "User");
        ...
        job1.setJarByClass(StoreItineraries.class);
        ...
        // Job2 Config
        conf.set(TableOutputFormat.OUTPUT_TABLE, "UserTimeItinerary");
        Job job2 = Job.getInstance(conf,"WriteResulttoHBase");
        job2.setJarByClass(StoreItineraries.class);
        ...
        while (true) {
            if (jobCtrl.allFinished()) {
                System.out.println("Job Succeess: "+jobCtrl.getSuccessfulJobList());
                jobCtrl.stop();
                break;
            }
        }
